...
export default function AdminDashboard() {
  ...
  return <>
    ...
    <section className="panel"><div className="panel-head"><div><h2>Total sales</h2><p>Calculated from live orders</p></div><strong>Rs. {totalSales.toFixed(2)}</strong></div></section>
    <section className="panel"><div className="panel-head"><div><h2>Recent orders</h2><p>Latest live order activity</p></div></div><DataTable rows={orders.slice(0, 10)} columns={[{key:'id',label:'ORDER ID',render:r=><b>{r.id}</b>},{key:'customer',label:'CUSTOMER'},{key:'date',label:'DATE'},{key:'total',label:'TOTAL',render:r=>`Rs. ${Number(r.total || 0).toFixed(2)}`},{key:'payment',label:'PAYMENT',status:true},{key:'status',label:'STATUS',status:true}]} /></section>
